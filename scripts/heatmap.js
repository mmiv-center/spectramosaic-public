...
let viewR = function(p) {
    ...
    p.drawXAxis = function() {
        ...
        for (var i = 0; i < p.xGroups.length; i++) {
            groupString += app.voxel_groups[p.xGroups[i]].name + "; ";
        }

        p.text("Groups on X axis: " + groupString, p.margin.left * 0.1, p.height * 0.9, p.width - p.margin.left * 0.1, p.height * 0.05);
    ...
    p.drawYAxis = function() {
        ...
        for (var i = 0; i < p.yGroups.length; i++) {
            groupString += app.voxel_groups[p.yGroups[i]].name + "; ";
        }

        p.text("Groups on Y axis: " + groupString, p.margin.left * 0.1, p.margin.top * 0.1, p.width - p.margin.left * 0.1, p.height * 0.05);
        ...
        for (var x = 0; x < p.xMetabolites.length; x++) {
            for (var y = 0; y < p.yMetabolites.length; y++) {
                ...
                for (var gx of p.xGroups) {
                    if (app.voxel_groups[gx].available_metabolites.includes(p.xMetabolites[x])) {
                        tile_avg_x += app.voxel_groups[gx].metabolite_averages[p.xMetabolites[x]].concentration;
                        error_warning = (error_warning || app.voxel_groups[gx].metabolite_averages[p.xMetabolites[x]].CRLB_percent_max > CRLB_WARNING_VAL);
                    }
                }
                for (var gy of p.yGroups) {
                    if (app.voxel_groups[gy].available_metabolites.includes(p.yMetabolites[y])) {
                        tile_avg_y += app.voxel_groups[gy].metabolite_averages[p.yMetabolites[y]].concentration;
                        error_warning = (error_warning || app.voxel_groups[gy].metabolite_averages[p.yMetabolites[y]].CRLB_percent_max > CRLB_WARNING_VAL);
                    }
                }
    ...
    p.drawExpandedDetails = function(tile_fill_color, mouse_over_tile) {
        ...
        p.highlightVoxels(ids_to_highlight);
        ...
        if (tooltip_rat != -1) {
            ...
            var error_warning = false;    
            for (var gx of p.xGroups) {
                if (app.voxel_groups[gx].available_metabolites.includes(p.xMetabolites[p.tileExpanded.x])) {
                    tile_avg_x += app.voxel_groups[gx].metabolite_averages[p.xMetabolites[p.tileExpanded.x]].concentration;
                    error_warning = (error_warning || app.voxel_groups[gx].metabolite_averages[p.xMetabolites[p.tileExpanded.x]].CRLB_percent > CRLB_WARNING_VAL);
                }
            }
            for (var gy of p.yGroups) {
                if (app.voxel_groups[gy].available_metabolites.includes(p.yMetabolites[p.tileExpanded.y])) {
                    tile_avg_y += app.voxel_groups[gy].metabolite_averages[p.yMetabolites[p.tileExpanded.y]].concentration;
                    error_warning = (error_warning || app.voxel_groups[gy].metabolite_averages[p.yMetabolites[p.tileExpanded.y]].CRLB_percent > CRLB_WARNING_VAL);
                }
            }
    ...
    p.highlightVoxels = function(vox_ids) {

        if (ALL_VOXELS_GROUP == -1) return; // this should not happen, but just in case

        // need to highlight the selected_voxels array (for left spectral view) and the all voxels group (for data tables) 

        app.voxel_groups[ALL_VOXELS_GROUP].voxels.forEach(function(voxel){
            idx = vox_ids.findIndex(function (id) {
                return id.includes("_" + voxel.vox_id);
            });

            if (idx != -1) voxel.highlighted = true;
            else voxel.highlighted = false;
        });
    ...
    p.drawXValues = function() {
        ...
        for (var x = 0; x < p.xMetabolites.length; x++) {
            ...
            for (var gx of p.xGroups) {
                ...
                if (app.voxel_groups[gx].voxels.length > 1) {
                    draw_range = true;
                }
                if (app.voxel_groups[gx].voxels.length > 4) {
                    draw_box_plot = true;
                }
                if (app.voxel_groups[gx].available_metabolites.includes(p.xMetabolites[x])) {
                    concentration_avg += app.voxel_groups[gx].metabolite_averages[p.xMetabolites[x]].concentration;
                    stddev_avg += app.voxel_groups[gx].metabolite_averages[p.xMetabolites[x]].std_deviation;

                    median += app.voxel_groups[gx].metabolite_averages[p.xMetabolites[x]].median;
                    min += app.voxel_groups[gx].metabolite_averages[p.xMetabolites[x]].min;
                    max += app.voxel_groups[gx].metabolite_averages[p.xMetabolites[x]].max;
                    q1 += app.voxel_groups[gx].metabolite_averages[p.xMetabolites[x]].q1;
                    q3 += app.voxel_groups[gx].metabolite_averages[p.xMetabolites[x]].q3;
    ...
    p.drawYValues = function() {
        ...
        for (var y = 0; y < p.yMetabolites.length; y++) {
            ...
            for (var gy of p.yGroups) {
                ...
                if (app.voxel_groups[gy].voxels.length > 1) {
                    draw_range = true;
                }

                if (app.voxel_groups[gy].voxels.length > 4) {
                    draw_box_plot = true;
                }

                if (app.voxel_groups[gy].available_metabolites.includes(p.yMetabolites[y])) {
                    concentration_avg += app.voxel_groups[gy].metabolite_averages[p.yMetabolites[y]].concentration;
                    stddev_avg += app.voxel_groups[gy].metabolite_averages[p.yMetabolites[y]].std_deviation;

                    median += app.voxel_groups[gy].metabolite_averages[p.yMetabolites[y]].median;
                    min += app.voxel_groups[gy].metabolite_averages[p.yMetabolites[y]].min;
                    max += app.voxel_groups[gy].metabolite_averages[p.yMetabolites[y]].max;
                    q1 += app.voxel_groups[gy].metabolite_averages[p.yMetabolites[y]].q1;
                    q3 += app.voxel_groups[gy].metabolite_averages[p.yMetabolites[y]].q3;
                }
            }
